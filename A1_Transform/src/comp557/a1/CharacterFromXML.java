...
public class CharacterFromXML {
	...
	public static GraphNode createJoint( Node dataNode ) {
		...
		if ( type.equals("freejoint") ) {
			...
			double min = Double.parseDouble(dataNode.getAttributes().getNamedItem("min").getNodeValue());
			double max = Double.parseDouble(dataNode.getAttributes().getNamedItem("max").getNodeValue());
			RotaryJoint joint = new RotaryJoint( name, min, max );
