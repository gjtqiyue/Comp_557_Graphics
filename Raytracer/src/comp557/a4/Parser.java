...
public class Parser {
	...
	public static Material createMaterial(Node dataNode) {
		Node refAttr = dataNode.getAttributes().getNamedItem("ref");
		if( refAttr  != null ) {
			Material material = Material.materialMap.get( refAttr.getNodeValue() );
			return material;
		} else {
			Node typeAttr = dataNode.getAttributes().getNamedItem("type");
	    	if ( typeAttr != null &&  MaterialType.valueOf(typeAttr.getNodeValue()) == MaterialType.EMISSIVE) {
	    		EmissiveMaterial e_mat = new EmissiveMaterial();
	    		Node nameAttr = dataNode.getAttributes().getNamedItem("name");
		    	if ( nameAttr != null ) {
		    		e_mat.name = nameAttr.getNodeValue();
		    	}
		    	Node colorAttr = dataNode.getAttributes().getNamedItem("color");
		    	if ( colorAttr != null ) {
		    		Scanner s = new Scanner( colorAttr.getNodeValue() );
		            ...
		            e_mat.color.set(r,g,b,a);
		    	}
		    	Node powerAttr = dataNode.getAttributes().getNamedItem("power");
		    	if ( powerAttr != null ) {
		    		e_mat.power = Float.parseFloat( powerAttr.getNodeValue() );
		    	}
		    	e_mat.type = MaterialType.valueOf(typeAttr.getNodeValue());
		    	...
		    	if ( diffuseAttr != null ) {
		            ...
		            s.close();
		            material.type = MaterialType.valueOf("DIFFUSE");
		    	}
		    	...
		    	if ( specularAttr != null ) {
		            ...
		            s.close();
		            material.type = MaterialType.valueOf("SPECULAR");
		    	}
		    	...
		    	Node reflectanceAttr = dataNode.getAttributes().getNamedItem("reflectance");
		    	if (reflectanceAttr != null ) {
		    		material.reflectance = Float.parseFloat( reflectanceAttr.getNodeValue() );
		    	}
		    	Node refractanceAttr = dataNode.getAttributes().getNamedItem("refractance");
		    	if (refractanceAttr != null ) {
		    		material.refractance = Float.parseFloat( refractanceAttr.getNodeValue() );
		    	}
		    	if ( typeAttr != null ) {
		    		material.type = MaterialType.valueOf(typeAttr.getNodeValue());
		    	}
	...
	public static Render createRender(Node dataNode) {
		...
		Node pathAttr = dataNode.getAttributes().getNamedItem("pathTracing");
		if ( pathAttr != null ) {
        	Scanner s = new Scanner( pathAttr.getNodeValue());
            int a = s.nextInt(); 
			s.close();
			render.pathTracing = a == 1 ? true : false;
		}
		Node depthAttr = dataNode.getAttributes().getNamedItem("depth");
		if ( depthAttr != null ) {
        	Scanner s = new Scanner( depthAttr.getNodeValue());
            render.depth = s.nextInt(); 
			s.close();
		}
		Node rrDepthAttr = dataNode.getAttributes().getNamedItem("rr_depth");
		if ( rrDepthAttr != null ) {
        	Scanner s = new Scanner( rrDepthAttr.getNodeValue());
            render.rr_depth = s.nextInt(); 
			s.close();
		}
		Node rrProbAttr = dataNode.getAttributes().getNamedItem("rr_prob");
		if ( rrProbAttr != null ) {
        	Scanner s = new Scanner( rrProbAttr.getNodeValue());
            render.rr_prob = s.nextFloat(); 
			s.close();
		}
    	NodeList nodeList = dataNode.getChildNodes();
    	for (int i = 0; i < nodeList.getLength(); i++) {
		...
		Node dofAttr = dataNode.getAttributes().getNamedItem("dof");
		if ( dofAttr != null ) {
        	Scanner s = new Scanner( dofAttr.getNodeValue());
            int a = s.nextInt(); 
			s.close();
			render.depthOfField = a == 1 ? true : false;
		}
		return render;
	}
